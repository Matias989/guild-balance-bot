...
/** Listado inline: icono + rol + cupo, con @menciones debajo */
export function buildAvalonianaRosterDescription(participants, emojiMap = null) {
  ...
  for (const role of AVALONIANA_ROLES) {
    ...
    const icon = emojiMap?.[role.name]?.toString?.() ?? role.icon;
    const lines = [`${icon} **${role.name}** (${count}/${role.max})`];
    if (members.length) {
